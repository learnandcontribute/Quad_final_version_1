...
// ═══ BOOKING MODAL — dvo-modalni: individualna vožnja + vođena tura ═══
const QUADS = {
  segway:     { half: 130, full: 200, max: 4, label: 'Segway AT5L' },
  linhai550:  { half: 130, full: 200, max: 2, label: 'Linhai 550L' },
  linhai650:  { half: 180, full: 250, max: 2, label: 'Linhai 650 Landforce' },
  linhai1000: { half: 250, full: 350, max: 2, label: 'Linhai 1000' },
  kids:       { half: 100, full: 150, max: 2, label: 'Dječji quad' },
  kids3h:     { fixed: 60,             max: 2, label: 'Dječji quad — 3h kraća' }
};
...
// Return the effective price per quad — respects fixed-price entries (e.g. kids3h)
function priceFor(quadKey) {
  const q = QUADS[quadKey];
  if (!q) return 0;
  return (typeof q.fixed === 'number') ? q.fixed : (q[booking.duration] || 0);
}

let booking = {
  mode: 'individual',   // 'individual' | 'tour'
  counts: { segway: 0, linhai550: 0, linhai650: 0, linhai1000: 0, kids: 0, kids3h: 0 },
  tourCount: 0,
  duration: 'half',
  date: ''
};
...
function openBooking(prefillType, prefillDuration) {
  ...
  // Odaberi početni mod: prvo iz body[data-booking-mode], inače 'individual'
  const bodyMode = document.body.getAttribute('data-booking-mode');
  const startMode = (bodyMode === 'tour') ? 'tour' : 'individual';

  // Prefill 1 of the requested type (resolving legacy aliases) — samo za individualnu vožnju
  if (prefillType && startMode === 'individual') {
    const resolved = PREFILL_ALIASES[prefillType] || prefillType;
    if (QUADS[resolved]) booking.counts[resolved] = 1;
  }
  // Ako otvaramo u tour modu, počni s 1 vozilom
  if (startMode === 'tour') booking.tourCount = 1;

  booking.duration = (prefillDuration === 'full') ? 'full' : 'half';
  setDuration(booking.duration);
  updateAllCounts();
  updateTourCountDisplay();
  setBookingMode(startMode);  // ovo također poziva updateSummary()
...
function setBookingMode(mode) {
  ...
  document.querySelectorAll('[data-mode-section]').forEach(sec => {
    sec.style.display = sec.dataset.modeSection === mode ? '' : 'none';
  });
  updateSummary();
}
...
function changeTourCount(delta) {
  booking.tourCount = Math.max(0, Math.min(TOUR_MAX, booking.tourCount + delta));
  updateTourCountDisplay();
  updateSummary();
}

function updateTourCountDisplay() {
  const el = document.getElementById('tourCountVal');
  if (el) el.textContent = booking.tourCount;
}
...
function updateSummary() {
  ...
  if (booking.mode === 'tour') {
    if (booking.tourCount > 0) {
      hasAny = true;
      total  = booking.tourCount * TOUR_PRICE;
      const people = booking.tourCount * 2;
      const row = document.createElement('div');
      row.className = 'modal__summary-row';
      row.innerHTML = '<span>' + booking.tourCount + '× Vođena tura <small style="color:var(--text-3)">(do ' + people + ' os.)</small></span><span>€' + total + '</span>';
      if (totalEl) summaryEl.insertBefore(row, totalEl);
      else summaryEl.appendChild(row);
    }
    if (emptyEl) emptyEl.textContent = 'Odaberite broj vozila';
  } else {
    for (const k in booking.counts) {
      const count = booking.counts[k];
      if (count > 0) {
        hasAny = true;
        const lineTotal = count * priceFor(k);
        total += lineTotal;
        const row = document.createElement('div');
        row.className = 'modal__summary-row';
    ...
    if (emptyEl) emptyEl.textContent = 'Dodajte barem jedan quad';
  }
...
function sendWhatsApp() {
  ...
  // ── TOUR MODE ──
  if (booking.mode === 'tour') {
    if (booking.tourCount <= 0) return;
    const total  = booking.tourCount * TOUR_PRICE;
    const people = booking.tourCount * 2;

    let msg = 'Bok! Želim rezervirati vođenu turu:\n\n';
    msg += '🗺 Tip: Vođena tura s vodičem\n';
    msg += '🏍 Broj vozila: ' + booking.tourCount + ' (do ' + people + ' osoba)\n';
